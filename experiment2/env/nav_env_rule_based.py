...
from turtlebot4_rl.rule_based_env import RuleBasedSubgoalGenerator
...
SUBGOAL_CLOSE_TO_GOAL_THRESHOLD = 0.3  # 若模型预测子目标距全局终点小于该值，则直接使用全局终点
SUBGOAL_MAX_DISTANCE = 2.0  # 若模型预测子目标距离当前位置过远，则沿方向截断到该距离
SUBGOAL_VALID_BACKOFF_STEP = 0.05  # 模型子目标不合法时，沿方向回退搜索的步长（米）
...
class TurtleBotNavEnv(gym.Env):
    def __init__(
        ...
        if self.seed is not None:
            self.action_space.seed(int(self.seed))
            self.observation_space.seed(int(self.seed))
        ...
        if self.subgoal_mode == "model" and subgoal_model_path:
    ...
    def _set_seed(self, seed: Optional[int]):
        """Set process-level RNG seeds used by position/yaw sampling and Torch inference."""
        if seed is None:
            return
        seed = int(seed)
        random.seed(seed)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if dist_to_global_goal < SUBGOAL_STOP_GENERATION_DISTANCE:
            ...
            if self.subgoal_mode == "model" and self.subgoal_model is not None:
        ...
        if subgoal is not None:
            ...
            close_to_goal_threshold = (
                SUBGOAL_CLOSE_TO_GOAL_THRESHOLD
                if self.subgoal_mode == "model"
                else SUBGOAL_SWITCH_THRESHOLD
            )

            # 子目标太近全局终点 -> 直接用全局终点
            if dist_subgoal_to_global < close_to_goal_threshold:
                self._print_and_log(
            ...
            if self.subgoal_mode in ("rule", "model"):
    ...
    def _is_position_valid(self, position: np.ndarray) -> bool:
        ...
        try:
            ...
            if not (np.isfinite(x) and np.isfinite(y)):
                return False
            return bool(is_position_valid(x, y, self.obstacles, bounds=self.map_bounds))
        except Exception:
            return False
    ...
    def step(self, action):
        ...
        # Get current state
        collision, _, min_lidar = self._is_collision()
        done = collision
        ...
        if self.using_subgoal and not target and not collision:
            ...
            if dist_to_global_goal < SUBGOAL_STOP_GENERATION_DISTANCE:
                self._print_and_log(
                    f"[Internal] Close to global goal (dist={dist_to_global_goal:.3f} < {SUBGOAL_STOP_GENERATION_DISTANCE}), stop generating subgoals."
                )
                self.current_goal = self.global_goal_position.copy()
                self.goal_position = self.current_goal
                self.using_subgoal = False
            # 到达当前子目标：生成下一个子目标（若允许）
            elif dist_to_current_goal < SUBGOAL_SWITCH_THRESHOLD:
                next_goal: Optional[np.ndarray] = None
                if self.subgoal_mode == "rule":
                    next_goal = self._generate_rule_subgoal()
                elif self.subgoal_mode == "model" and self.subgoal_model is not None:
                ...
                if next_goal is not None:
                    ...
                    if dist_next_to_global < close_to_goal_threshold or dist_next_to_robot < SUBGOAL_SWITCH_THRESHOLD:
                        self._print_and_log(
    ...
    def _cap_subgoal_distance(self, subgoal: np.ndarray, max_distance: float) -> np.ndarray:
        """Limit a model-predicted subgoal to max_distance from the current pose."""
        ...
        if not np.isfinite(distance) or distance <= 1e-9 or distance <= max_distance:
            return subgoal
    ...
    def _project_subgoal_to_valid(
        ...
        """Back off along the predicted direction until the subgoal is valid."""
        if subgoal is None or self.current_position is None:
            return None
        ...
        start_distance = min(distance, max_distance)
        candidate_distance = start_distance
        while candidate_distance >= min_distance - 1e-9:
            candidate = current + unit * candidate_distance
            if self._is_position_valid(candidate):
                if candidate_distance < start_distance - 1e-6:
                    self._print_and_log(
                        f"[Internal] Subgoal invalid; backed off to valid point at dist={candidate_distance:.3f}m (start={start_distance:.3f}m)."
                    )
